...
usage() {
  cat <<'EOF'
Usage:
  run_codex_supervised.sh --workdir /abs/path --prompt-file /abs/path/prompt.txt [options]
  run_codex_supervised.sh --workdir /abs/path --prompt 'do the thing' [options]
...
Options:
  ...
  --extra-launch-args ARG  Extra args appended to codex launch command

Notes:
  - Codex is launched directly in tmux with: export TERM=xterm-256color && codex --yolo
  - The supervisor auto-accepts Codex's initial trust prompt for the workdir when it appears.
  - Logs/status are written under ${HERMES_HOME}/agent-supervisor/<session>/
  - For job-name based management, use codex_job.py instead of this thin wrapper.
EOF
}
