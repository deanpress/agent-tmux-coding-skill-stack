...
4. Ensure the default Codex path uses vanilla `codex --yolo` for interactive tmux-supervised runs, and use plain `opencode` when the user explicitly asks for OpenCode.
5. For substantial coding tasks, use the tmux supervisor/job-manager flow rather than a one-shot `codex exec` or `opencode run`.
...
7. Verify the supervisor, check-in, resume, Codex initialization, and OpenCode docs are internally consistent.
